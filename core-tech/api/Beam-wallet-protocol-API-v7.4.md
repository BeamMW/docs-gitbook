...
- [sign_message](#sign_message) now accepts an optional `address` parameter. `key_material` is now optional (kept for backward compatibility). `message` is now optional — if omitted the signature proves ownership of the address with an empty message.

### New methods
- [verify_message](#verify_message) verifies a Schnorr signature against a wallet address (companion to `sign_message`)
- [send_message](#send_message) sends an instant SBBS message to another wallet ![](https://img.shields.io/badge/-DAPPs%20allowed-green)
- [read_messages](#read_messages) reads instant SBBS messages received by this wallet ![](https://img.shields.io/badge/-DAPPs%20allowed-green)
...
- [verify_message](#verify_message) ![](https://img.shields.io/badge/-DAPPs%20allowed-green)
- [derive_id](#derive_id) ![](https://img.shields.io/badge/-DAPPs%20allowed-green)
- [send_message](#send_message) ![](https://img.shields.io/badge/-DAPPs%20allowed-green)
- [read_messages](#read_messages) ![](https://img.shields.io/badge/-DAPPs%20allowed-green)
...
`wallet-api` must be started with `--enable_assets` key

- [assets_swap_offers_list](#assets_swap_offers_list)
- [assets_swap_create](#assets_swap_create)
- [assets_swap_cancel](#assets_swap_cancel)
- [assets_swap_accept](#assets_swap_accept)
...
* `use_default_signature` - if it's `true` then generated address will use default wallet signature. By default it's `false`.
...
```json
{
  ...
  "result": {
    ...
    "beam_version": "7.4.0",
    "beam_version_major": 7,
    "beam_version_minor": 4,
    "beam_version_rev": 0
  }
}
```
...
Generates a Schnorr signature for a message, proving ownership of a wallet address. The signature commits to SHA256(`"beam.signed.message"` + len(message) + message).
...
```json
{
    ...
    "params":
    {
        "address": "50614dccd139f398504882567e5246ecd2cbe031085be97806cea7f07510443c69",
        "message": "my message"
    }
}
```

* `address` hex-encoded SBBS wallet address (WalletID) to sign with. Optional — if omitted the wallet's default address is used. The address must be owned by this wallet.
* `message` the message to sign. Optional — if omitted an empty string is signed, which is sufficient to prove ownership of the address.
...
* `signature` 65-byte hex-encoded Schnorr signature (33 bytes nonce point + 32 bytes scalar)

**Backward compatibility:** The legacy `key_material` parameter (hex-encoded raw key bytes from a shader) is still accepted in place of `address`. If neither `address` nor `key_material` is provided the wallet's default address is used.
...
Verifies a Schnorr signature against a raw public key. See [verify_message](#verify_message) for the address-based variant.
...
* `public_key` 33-byte hex-encoded compressed secp256k1 public key
* `signature` 65-byte hex-encoded signature
...
Verifies a Schnorr signature produced by [sign_message](#sign_message) against a wallet address. The address encodes the public key directly, so no wallet access is required for verification.
...
* `address` hex-encoded SBBS wallet address (WalletID) whose public key is used to verify the signature. Does not need to be in the local wallet database.
* `message` the message that was signed. Must match the message used in `sign_message`. Optional, defaults to empty string.
* `signature` 65-byte hex-encoded signature returned by `sign_message`
...
* `is_valid` `true` if the signature is valid for the given address and message, `false` otherwise

**Round-trip example:**

`-->`
```json
{"jsonrpc":"2.0","id":1,"method":"sign_message","params":{"message":"hello beam"}}
```
`<--`
```json
...
```json
{
    "jsonrpc":"2.0","id":2,"method":"verify_message",
    "params":{
        "address":"<address from sign_message response or addr_list>",
        "message":"hello beam",
...
`<--`
```json
{"jsonrpc":"2.0","id":2,"result":{"is_valid":true}}
```
...
* `refresh` if it's `false` the locally stored info about assets is returned, otherwise the info about assets is downloaded from the node up to the given `height`. All downloaded info is stored locally, so, there is no need to `refresh` every time, this operation should be performed from time to time.
* `height` if `refresh` is `true` the list of assets registered up to this height is returned 
...
```json
{
  "assets": [
    {
      "asset_id": 1,
      "emission_str": "10000000000000000",
      "isOwned": 0,
      "lockHeight": 27683,
      "metadata": "STD:SCH_VER=1;N=BeamX Token;SN=BeamX;UN=BEAMX;NTHUN=GROTHX",
      "metadata_kv": true,
      "metadata_pairs": [Object],
      "metadata_std": true,
      "metadata_std_min": true,
      "metadata_v50": true,
      "metadata_v60": true,
      "ownerId": "0f3bdafc459a26b9884b480c755900b37d20cf88fbdf3edebeba260cffb6cdfd",
      "refreshHeight": 1932858
    },
    {
      "asset_id": 2,
      "emission_str": "10000000000000000",
      ...
      "metadata_v50": true,
      "metadata_v60": true,
      "ownerId": "c41f2c22ca4d924e79524e221538f65797e7b5878185c75a1380e7af8fa3a0a7",
      "refreshHeight": 1932858
    }
  ]
}
...
see [get_asset_info](#get_asset_info) for details of the output

## send_message

Sends an instant off-chain message to another wallet via the SBBS (Secure Bulletin Board System) network. The message is delivered directly to the recipient's SBBS address without creating an on-chain transaction. Messages are not stored on-chain and are lost if the recipient is offline when delivery is attempted.
...
* `receiver` hex-encoded destination SBBS address (WalletID). Mandatory.
* `message` a JSON object containing the message payload. The structure is application-defined — any valid JSON object is accepted. Mandatory.
* `sender` hex-encoded own SBBS address to send from. Optional — if omitted the wallet's default address is used.
...
```json
{
    ...
    "id": 1,
    "result": {
        "from": "22813f28ae198b1aa28aed3d0dc69a7c75f0a3c8b72c5950c997d0a569e74f90b61",
        "to": "50614dccd139f398504882567e5246ecd2cbe031085be97806cea7f07510443c69",
        "bytes": 27
    }
}
```

* `from` hex-encoded SBBS address the message was sent from
* `to` hex-encoded SBBS address the message was sent to
* `bytes` size of the serialized message payload in bytes

## read_messages

Returns instant SBBS messages received by this wallet.
...
* `all` optional bool. If `true` returns all stored messages. If `false` (default) returns only messages that have not been read yet.
    ...
    "result": [
        {
            ...
            "sender": "22813f28ae198b1aa28aed3d0dc69a7c75f0a3c8b72c5950c997d0a569e74f90b61",
            "message": {
                "type": "chat",
                "text": "Hello!"
            }
...
* `id` numeric message identifier, monotonically increasing per wallet database
* `timestamp` Unix timestamp (seconds) when the message was received
* `sender` hex-encoded SBBS address of the sender
* `message` the original JSON object that was passed as `message` in `send_message`

To read all messages including previously read ones:
